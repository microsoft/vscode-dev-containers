...
get_common_setting() {
    if [ "${common_settings_file_loaded}" != "true" ]; then
        echo curl -sfL "https://aka.ms/vscode-dev-containers/script-library/settings.env"
...
if type docker > /dev/null 2>&1 && type dockerd > /dev/null 2>&1; then
    ...
    if [ "${USE_MOBY}" = "true" ]; then
        ...
        echo curl -sSL ${MICROSOFT_GPG_KEYS_URI}
        ...
        # Import key safely (new 'signed-by' method rather than deprecated apt-key approach) and install
        echo curl -fsSL https://download.docker.com/linux/${ID}/gpg
...
if type docker-compose > /dev/null 2>&1; then
	...
	echo target_compose_arch $target_compose_arch
    ...
    if [ "${target_compose_arch}" != "x86_64" ]; then
        ...
        echo curl -fsSL "https://github.com/docker/compose/releases/download/${latest_compose_version}/docker-compose-$(uname -s)-${target_compose_arch}" -o /usr/local/bin/docker-compose
