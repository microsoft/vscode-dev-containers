...
# Install curl, lldb, python3-minimal,libpython and dotnet dependencies if missing
...
# Install dotnet: TODO
umask 0002
if ! cat /etc/group | grep -e "^${ACCESS_GROUP_NAME}:" > /dev/null 2>&1; then
    groupadd -r ${ACCESS_GROUP_NAME}
fi
usermod -a -G ${ACCESS_GROUP_NAME} "${USERNAME}"
...
# mkdir -p "${target_install_path}"
# Install goes here
# chown -R :${access_group_name} "${target_install_path}"
# find "${target_install_path}" -type d | xargs -n 1 chmod g+s
